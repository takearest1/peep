...
function move() {
    ...
    setTimeout(function () {
      ...
      if(random2 == 4){//手芸しに行く
        $('#character')
          .animate({'top':'45%','left':'60%'},2000);//ゴール地点 //移動にかかる時間
        setTimeout(function () {
            chara.innerHTML = '<img id="syugei_down" class="dark character" src="images/syugei_down.gif?' +(new Date).getTime()+ '">';
            $("#cloth").css('opacity', '0');
            setTimeout(function () {
              random = 4;
              syugei_f = false;
  ...
  if(random == 4){
    ...
    ///////立ち上がる動作
    chara.innerHTML = '<img id="syugei_up" class="dark character" src="images/syugei_up.gif?' +(new Date).getTime()+ '">';
      random = 0;
    setTimeout(function () {
      $("#cloth").css('opacity', '1');
      if(random2 == 6 && s_flag == false){
